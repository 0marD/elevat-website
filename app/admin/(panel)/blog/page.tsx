...
import { getAll, serialize } from '@/lib/data/blog-store'
import { ROUTES } from '@/lib/constants/routes'
import BlogTable from './BlogTable'

export const metadata: Metadata = { title: 'Blog — Panel ÉLEVA.' }

export const dynamic = 'force-dynamic'

export default async function AdminBlogPage() {
  const posts     = (await getAll()).map(serialize)
  ...
  return (
    <div className="p-6 md:p-10 max-w-6xl">
      ...
      <header className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
        ...
        >
          <span aria-hidden="true">+</span> Nuevo artículo
        </Link>
      </header>
      ...
      <div className="flex gap-4 mb-8 flex-wrap">
        ...
        <Metric label="Publicados"  value={publicados} highlight />
        <Metric label="Borradores"  value={borradores} />
      </div>
      ...
      <p className="mt-6 text-[10px] text-plata/30 tracking-wide">
        Los datos se guardan en <code className="text-dorado/50">lib/data/posts.json</code>.
